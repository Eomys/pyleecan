...
class MachineAsync(Machine):
    ...
    if isinstance(is_synchronous, ImportError):
        is_synchronous = property(fget=lambda x: raise_(ImportError("Can't use MachineAsync method is_synchronous: " + str(is_synchronous))))
    else:
    ...
    def __init__(self, rotor=-1, stator=-1, frame=-1, shaft=-1, name="default_machine", desc="", init_dict=None):
        ...
        # Call Machine init
        super(MachineAsync, self).__init__(rotor=rotor, stator=stator, frame=frame, shaft=shaft, name=name, desc=desc)
