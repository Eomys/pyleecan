...
try:
    from pyleecan.Methods.Machine.WindingCW2LR.comp_connection_mat import comp_connection_mat
...
class WindingCW2LR(Winding):
    ...
    if isinstance(comp_connection_mat, ImportError):
        comp_connection_mat = property(fget=lambda x: raise_(ImportError("Can't use WindingCW2LR method comp_connection_mat: " + str(comp_connection_mat))))
    else:
    ...
    if isinstance(get_dim_wind, ImportError):
        get_dim_wind = property(fget=lambda x: raise_(ImportError("Can't use WindingCW2LR method get_dim_wind: " + str(get_dim_wind))))
    else:
