...
class OptiProblem(FrozenClass):
    ...
    def __init__(self, output=-1, design_var=dict(), obj_func=dict(), eval_func=None, init_dict=None):
        ...
        if init_dict is not None:  # Initialisation by dict
            check_init_dict(init_dict, ["output", "design_var", "obj_func", "eval_func"])
    ...
    def __str__(self):
        ...
        if self.output is not None:
            OptiProblem_str += "output = " + str(self.output.as_dict()) + linesep + linesep
        else:
        ...
        for key, obj in self.design_var.items():
            OptiProblem_str += "design_var["+key+"] = "+str(self.design_var[key].as_dict()) + linesep + linesep
        if len(self.obj_func) == 0:
            OptiProblem_str += "obj_func = dict()" + linesep
        for key, obj in self.obj_func.items():
            OptiProblem_str += "obj_func["+key+"] = "+str(self.obj_func[key].as_dict()) + linesep + linesep
