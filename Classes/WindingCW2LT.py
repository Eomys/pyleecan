...
class WindingCW2LT(Winding):
    ...
    if isinstance(comp_connection_mat, ImportError):
        comp_connection_mat = property(fget=lambda x: raise_(ImportError("Can't use WindingCW2LT method comp_connection_mat: " + str(comp_connection_mat))))
    else:
    ...
    if isinstance(get_dim_wind, ImportError):
        get_dim_wind = property(fget=lambda x: raise_(ImportError("Can't use WindingCW2LT method get_dim_wind: " + str(get_dim_wind))))
    else:
    ...
    def __str__(self):
        ...
        WindingCW2LT_str += super(WindingCW2LT, self).__str__() + linesep
