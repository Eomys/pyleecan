...
class MachineWRSM(MachineSync):
    ...
    if isinstance(check, ImportError):
        check = property(fget=lambda x: raise_(ImportError("Can't use MachineWRSM method check: " + str(check))))
    else:
        check = check
    # cf Methods.Machine.MachineWRSM.get_machine_type
    if isinstance(get_machine_type, ImportError):
        get_machine_type = property(fget=lambda x: raise_(ImportError("Can't use MachineWRSM method get_machine_type: " + str(get_machine_type))))
    else:
    ...
    def __init__(self, rotor=-1, stator=-1, frame=-1, shaft=-1, name="default_machine", desc="", init_dict=None):
        ...
        super(MachineWRSM, self).__init__(rotor=rotor, stator=stator, frame=frame, shaft=shaft, name=name, desc=desc)
