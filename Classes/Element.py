...
try:
    from pyleecan.Methods.Mesh.Element.get_new_tag import get_new_tag
except ImportError as error:
    get_new_tag = error

try:
    from pyleecan.Methods.Mesh.Element.get_all_node_tags import get_all_node_tags
except ImportError as error:
...
class Element(FrozenClass):
    ...
    # cf Methods.Mesh.Element.get_new_tag
    if isinstance(get_new_tag, ImportError):
        get_new_tag = property(
            fget=lambda x: raise_(
                ImportError("Can't use Element method get_new_tag: " + str(get_new_tag))
            )
        )
    else:
        get_new_tag = get_new_tag
    # cf Methods.Mesh.Element.get_all_node_tags
    if isinstance(get_all_node_tags, ImportError):
        get_all_node_tags = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use Element method get_all_node_tags: "
