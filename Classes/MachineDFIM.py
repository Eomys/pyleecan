...
class MachineDFIM(MachineAsync):
    ...
    if isinstance(check, ImportError):
        check = property(fget=lambda x: raise_(ImportError("Can't use MachineDFIM method check: " + str(check))))
    else:
        check = check
    # cf Methods.Machine.MachineDFIM.get_machine_type
    if isinstance(get_machine_type, ImportError):
        get_machine_type = property(fget=lambda x: raise_(ImportError("Can't use MachineDFIM method get_machine_type: " + str(get_machine_type))))
    else:
