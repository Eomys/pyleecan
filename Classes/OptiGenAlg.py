...
class OptiGenAlg(FrozenClass):
    ...
    def __str__(self):
        ...
        if self.multi_output is not None:
            OptiGenAlg_str += "multi_output = " + str(self.multi_output.as_dict()) + linesep + linesep
        else:
        ...
        OptiGenAlg_str += "p_mute = " + str(self.p_mute) + linesep
        OptiGenAlg_str += "size_pop = " + str(self.size_pop) + linesep
        if self.problem is not None:
            OptiGenAlg_str += "problem = " + str(self.problem.as_dict()) + linesep + linesep
        else:
    ...
    def as_dict(self):
        ...
        OptiGenAlg_dict["p_mute"] = self.p_mute
    ...
    def _get_p_mute(self):
        """getter of p_mute"""
        return self._p_mute

    def _set_p_mute(self, value):
        """setter of p_mute"""
        check_var("p_mute", value, "float", Vmin=0, Vmax=1)
        self._p_mute = value

    # Probability of mutation 
    # Type : float, min = 0, max = 1
    p_mute = property(
        fget=_get_p_mute, fset=_set_p_mute, doc=u"""Probability of mutation """
    )
