...
try:
    from pyleecan.Methods.Machine.WindingCW1L.comp_connection_mat import comp_connection_mat
except ImportError as error:
    comp_connection_mat = error

try:
    from pyleecan.Methods.Machine.WindingCW1L.get_dim_wind import get_dim_wind
except ImportError as error:
    get_dim_wind = error
...
class WindingCW1L(Winding):
    ...
    if isinstance(comp_connection_mat, ImportError):
        comp_connection_mat = property(fget=lambda x: raise_(ImportError("Can't use WindingCW1L method comp_connection_mat: " + str(comp_connection_mat))))
    else:
        comp_connection_mat = comp_connection_mat
    # cf Methods.Machine.WindingCW1L.get_dim_wind
    if isinstance(get_dim_wind, ImportError):
        get_dim_wind = property(fget=lambda x: raise_(ImportError("Can't use WindingCW1L method get_dim_wind: " + str(get_dim_wind))))
    else:
    ...
    def __init__(self, is_reverse_wind=False, Nslot_shift_wind=0, qs=3, Ntcoil=7, Npcpp=2, type_connection=0, p=3, Lewout=0.015, conductor=-1, init_dict=None):
        ...
        # Call Winding init
        super(WindingCW1L, self).__init__(is_reverse_wind=is_reverse_wind, Nslot_shift_wind=Nslot_shift_wind, qs=qs, Ntcoil=Ntcoil, Npcpp=Npcpp, type_connection=type_connection, p=p, Lewout=Lewout, conductor=conductor)
    ...
    def __str__(self):
        ...
        WindingCW1L_str += super(WindingCW1L, self).__str__() + linesep
