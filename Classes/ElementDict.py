...
try:
    from pyleecan.Methods.Mesh.ElementDict.get_element import get_element
except ImportError as error:
    get_element = error
...
class ElementDict(Element):
    ...
    if isinstance(get_group, ImportError):
        get_group = property(fget=lambda x: raise_(ImportError("Can't use ElementDict method get_group: " + str(get_group))))
    else:
        get_group = get_group
    # cf Methods.Mesh.ElementDict.get_element
    if isinstance(get_element, ImportError):
        get_element = property(fget=lambda x: raise_(ImportError("Can't use ElementDict method get_element: " + str(get_element))))
    else:
        get_element = get_element
    # cf Methods.Mesh.ElementDict.get_node2element
    if isinstance(get_node2element, ImportError):
        get_node2element = property(fget=lambda x: raise_(ImportError("Can't use ElementDict method get_node2element: " + str(get_node2element))))
    else:
        get_node2element = get_node2element
    # cf Methods.Mesh.ElementDict.add_element
    if isinstance(add_element, ImportError):
        add_element = property(fget=lambda x: raise_(ImportError("Can't use ElementDict method add_element: " + str(add_element))))
    else:
        add_element = add_element
    # cf Methods.Mesh.ElementDict.convert_element
    if isinstance(convert_element, ImportError):
        convert_element = property(fget=lambda x: raise_(ImportError("Can't use ElementDict method convert_element: " + str(convert_element))))
    else:
    ...
    def __str__(self):
        ...
        ElementDict_str += "group = " + linesep + str(self.group) + linesep + linesep
        ElementDict_str += "nb_elem = " + str(self.nb_elem) + linesep
        ElementDict_str += "nb_node_per_element = " + str(self.nb_node_per_element)
    ...
    def as_dict(self):
        ...
        if self.group is None:
            ElementDict_dict["group"] = None
        else:
            ElementDict_dict["group"] = self.group.tolist()
    ...
    # Dict containing tags and connectivity strored per element type in a ndarray: element = {"Triangle": np.array([tag_elem, nd1, nd2, nd3 ]), "Quadrangle": np.array([tag_elem, nd1, nd2, nd3, nd4 ])}.
    # Type : dict
    connectivity = property(fget=_get_connectivity, fset=_set_connectivity,
                            doc=u"""Dict containing tags and connectivity strored per element type in a ndarray: element = {"Triangle": np.array([tag_elem, nd1, nd2, nd3 ]), "Quadrangle": np.array([tag_elem, nd1, nd2, nd3, nd4 ])}.""")
    ...
    # Attribute a group number (int) to each element: group = np.array([tag_elem, group]) . This group number should correspond to a subpart of the machine.
    # Type : ndarray
    group = property(fget=_get_group, fset=_set_group,
                     doc=u"""Attribute a group number (int) to each element: group = np.array([tag_elem, group]) . This group number should correspond to a subpart of the machine.""")
