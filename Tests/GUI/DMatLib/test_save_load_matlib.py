...
class test_save_load_matlib(TestCase):
    ...
    def test_load_save_several_file(self):
        ...
        # Start the GUI
        self.mat_widget = DMatLib(matlib_path=self.work_path)
        self.widget = DMachineSetup(
            mat_widget=self.mat_widget, machine=None, machine_path=self.work_path
        )
        # Check load of the matlib
        self.assertEqual(len(self.mat_widget.matlib), 4)
        self.assertEqual(
            ["Copper1", "Insulator1", "M400-50A", "Magnet1"],
            [mat.name for mat in self.mat_widget.matlib],
        )
        self.assertEqual(self.mat_widget.matlib[0].elec.rho, 1.73e-8)
        self.assertEqual(self.mat_widget.matlib[0].HT.alpha, 0.00393)
        self.assertEqual(
            self.mat_widget.matlib[0].path, join(self.work_path, "Copper1.json")
        )

        self.assertEqual(self.mat_widget.matlib[2].mag.mur_lin, 2500)
        self.assertEqual(self.mat_widget.matlib[2].struct.rho, 7650)
        self.assertEqual(self.mat_widget.matlib[2].struct.Ex, 215000000000)
        self.assertEqual(
            self.mat_widget.matlib[2].path,
            join(self.work_path, "Lamination", "M400-50A.json"),
        )
        # Change value of materials
        self.mat_widget.matlib[0].elec.rho = 1.74e-8
        self.mat_widget.matlib[0].HT.alpha = 0.00555
        self.mat_widget.matlib[2].mag.mur_lin = 2501.2
        self.mat_widget.matlib[2].struct.rho = 76
        # Save matlib
        for mat in self.mat_widget.matlib:
            mat.save(mat.path)
            mat2 = load(mat.path)
