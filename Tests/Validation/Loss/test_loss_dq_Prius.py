...
def test_FEMM_Loss_Prius():
    ...
    for i, out in enumerate(xout):
        if not exists(fr"C:\Users\LAP10\Documents\Loss\plot_B_mesh\Id={OP_MATRIX[i,1]}, Iq={OP_MATRIX[i,2]}.gif"):
            out.plot_B_mesh(is_animated=True, save_path = fr"C:\Users\LAP10\Documents\Loss\plot_B_mesh\Id={OP_MATRIX[i,1]}, Iq={OP_MATRIX[i,2]}.gif",)
        out.plot_B_mesh()
        power_dict = {
    ...
    for i, out in enumerate(out_list[:-1]):
        for j, other in enumerate(out_list[i+1:],start=i+1):
            ...
            out.loss.loss_list[-1].name = f"(Id={OP_MATRIX[i,1]}, Iq={OP_MATRIX[i,2]}) - (Id={OP_MATRIX[j,1]}, Iq={OP_MATRIX[j,2]})"
