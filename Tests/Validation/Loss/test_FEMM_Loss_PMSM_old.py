...
def test_FEMM_Loss_Prius():
    ...
    power_dict = {
        "total_power": out.mag.Pem_av,
        "copper_loss": out.loss.Pjoule,
        "iron_loss": out.loss.Pstator + out.loss.Protor,
        "magnet_loss": out.loss.Pmagnet,
    }
    print(power_dict)
    with open("first_simu_prius.json","w") as f:
    ...
    #     # clim=[2e4, 2e7],
    # )
    out.mag.meshsolution.solution[0].plot_contour(
        "freqs=sum",
        label="B",
        group_names=["stator core", "stator winding"],
        # clim=[2e4, 2e7],
    ...
    out.mag.meshsol_list[0].plot_contour(
        "freqs=sum",
        label="B",
        group_names=["rotor core", "rotor magnets"],
        # clim=[2e4, 2e7],
