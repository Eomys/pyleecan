...
@pytest.mark.MeshSol
class unittest_ref_points(TestCase):
    ...
    def test_line2(self):
        ...
        test = np.array([0.5, 0])
        ref_points = c_line.interpolation.ref_cell.get_ref_point(vert, test)
        testA = np.sum(abs(solution - ref_points))
        msg = (
            "Wrong result: returned " + str(ref_points) + ", expected: " + str(solution)
        )
