...
def merge_slot_intersect(self, radius_desc_list, prop_dict, sym):
    ...
    for desc_dict in radius_desc_list:
        if desc_dict["label"] == "Radius":
            ...
            # define cutting line
            if len(line_list) > 0:  # line_list == 0 start with cut slot/notch
                # Define First cutting line
                Z1 = line_list[-1].get_end()
                Z2 = (Z1 * exp(-1j * angle(Z1)) - 1) * exp(1j * angle(Z1))
                # Find first line to intersect with cutting line
                for ii in range(len(desc_dict["lines"])):
                    if desc_dict["lines"].intersect(Z1, Z2) > 0:
                        break
                if ii < len(desc_dict["lines"]):
                    # Slot/notch was cut
                    lines = desc_dict["lines"][ii:]
                    top, bot = lines[0].split_line(Z1, Z2)
                    lines[0] = top
