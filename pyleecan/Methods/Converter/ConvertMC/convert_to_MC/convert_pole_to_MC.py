def convert_pole_to_MC(self):
    """Selects correct pole and implements it in dict

    Parameters
    ----------
    self : ConvertMC
        A ConvertMC object

    Returns
    ---------

    """
    # conversion to motor-cad
    slot_type = type(self.machine.rotor.slot).__name__

    # selection type of pole
    if slot_type == "SlotW62":
        name_hole = "Sync_Salient_Pole"

    elif slot_type == "SlotW63":
        name_hole = "Sync_Parallel_Tooth"

    elif slot_type == "SlotW29":
        name_hole = "Sync_Parallel_Slot"

    else:
        raise NotImplementedError(
            f"Type of hole {slot_type} has not equivalent or has not implement"
        )

    # writting in dict
    if "[Design_Options]" not in self.other_dict:
        self.other_dict["[Design_Options]"] = {"Sync_Rotor": name_hole}
    else:
        self.other_dict["[Design_Options]"]["Sync_Rotor"] = name_hole

    self.get_logger().info(f"Conversion {slot_type} into {name_hole}")
