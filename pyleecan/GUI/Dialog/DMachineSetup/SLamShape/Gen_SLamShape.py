...
class Gen_SLamShape(Ui_SLamShape):
    def setupUi(self, SLamShape):
        ...
        # Setup of in_L1
        txt = self.tr(
            """<qt><nobr>Lamination stack active length without radial</nobr> ventilation airducts but including insulation layers between lamination sheets</qt>"""
        )
        ...
        # Setup of in_Kf1
        txt = self.tr("""<qt><nobr>lamination stacking / packing factor</nobr></qt>""")
        ...
        # Setup of w_mat
        txt = self.tr("""<qt><nobr>Lamination's material</nobr></qt>""")
        self.w_mat.setWhatsThis(txt)
        self.w_mat.setToolTip(txt)

        # Setup of in_Wrvd
        txt = self.tr(
            """<qt><nobr>axial width of ventilation ducts in lamination</nobr></qt>"""
        )
        ...
        # Setup of in_Nrvd
        txt = self.tr(
            """<qt><nobr>number of radial air ventilation ducts in lamination</nobr></qt>"""
        )
