...
from ......GUI.Dialog.DMachineSetup.SWSlot.PWSlot30.Gen_PWSlot30 import Gen_PWSlot30
...
class PWSlot30(Gen_PWSlot30, QWidget):
    """Page to set the Slot Type 30"""

    # Signal to DMachineSetup to know that the save popup is needed
    saveNeeded = Signal()
    # Information for Slot combobox
    slot_name = "Slot Type 30"
    slot_type = SlotW30
    ...
    def set_wedge(self):
        ...
        if self.g_wedge.isChecked():
            self.img_slot.setPixmap(
                QPixmap(pixmap_dict["SlotW30_wedge_full_ext_stator"])
            )
            self.w_wedge_mat.update(self.slot, "wedge_mat", self.material_dict)
        else:
            self.slot.wedge_mat = None
            self.img_slot.setPixmap(QPixmap(pixmap_dict["SlotW30_wind_ext_stator"]))
