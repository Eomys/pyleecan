...
try:
    from ..Methods.Slot.SlotW27.get_surface_wind import get_surface_wind
except ImportError as error:
...
class SlotW27(SlotWind):
    ...
    # cf Methods.Slot.SlotW27.get_surface_wind
    if isinstance(get_surface_wind, ImportError):
        get_surface_wind = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW27 method get_surface_wind: " + str(get_surface_wind)
                )
            )
        )
    else:
    ...
    def __init__(self, H0=0.003, H1=0, H2=0.02, W0=0.003, W1=0.013, W2=0.01, W3=0.003, is_trap_wind=False, Zs=36, init_dict = None, init_str = None):
