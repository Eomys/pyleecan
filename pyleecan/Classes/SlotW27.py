...
try:
    from ..Methods.Slot.SlotW27._comp_line_dict import _comp_line_dict
...
try:
    from ..Methods.Slot.SlotW27.comp_surface_opening import comp_surface_opening
except ImportError as error:
...
try:
    from ..Methods.Slot.SlotW27.get_surface_opening import get_surface_opening
except ImportError as error:
...
class SlotW27(Slot):
    ...
    # cf Methods.Slot.SlotW27._comp_line_dict
    if isinstance(_comp_line_dict, ImportError):
        _comp_line_dict = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW27 method _comp_line_dict: " + str(_comp_line_dict)
                )
            )
        )
    else:
    ...
    # cf Methods.Slot.SlotW27.comp_surface_opening
    if isinstance(comp_surface_opening, ImportError):
        comp_surface_opening = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW27 method comp_surface_opening: "
    ...
    # cf Methods.Slot.SlotW27.get_surface_opening
    if isinstance(get_surface_opening, ImportError):
        get_surface_opening = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW27 method get_surface_opening: "
    ...
    def __init__(self, H0=0.003, H1=0, H2=0.02, W0=0.003, W1=0.013, W2=0.01, W3=0.003, is_trap_wind=False, Zs=36, wedge_mat=None, is_bore=True, init_dict = None, init_str = None):
    ...
    def compare(self, other, name='self', ignore_list=None, is_add_value=False):
        ...
        # Check the properties inherited from Slot
        diff_list.extend(super(SlotW27, self).compare(other,name=name, ignore_list=ignore_list, is_add_value=is_add_value))
        ...
        if other._is_trap_wind != self._is_trap_wind:
            if is_add_value:
                val_str = ' (self='+str(self._is_trap_wind)+', other='+str(other._is_trap_wind)+')'
                diff_list.append(name+'.is_trap_wind'+val_str)
            else:
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        # Get the properties inherited from Slot
        SlotW27_dict = super(SlotW27, self).as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs)
    ...
    def copy(self):
        ...
        W2_val = self.W2
        W3_val = self.W3
        is_trap_wind_val = self.is_trap_wind
        Zs_val = self.Zs
        ...
        obj_copy = type(self)(H0=H0_val,H1=H1_val,H2=H2_val,W0=W0_val,W1=W1_val,W2=W2_val,W3=W3_val,is_trap_wind=is_trap_wind_val,Zs=Zs_val,wedge_mat=wedge_mat_val,is_bore=is_bore_val)
        return obj_copy
