...
try:
    from ..Methods.Machine.CondType22.is_round_wires import is_round_wires
except ImportError as error:
...
class CondType22(Conductor):
    ...
    # cf Methods.Machine.CondType22.is_round_wires
    if isinstance(is_round_wires, ImportError):
        is_round_wires = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use CondType22 method is_round_wires: " + str(is_round_wires)
                )
            )
        )
    else:
