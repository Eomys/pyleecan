# -*- coding: utf-8 -*-
# File generated according to Generator/ClassesRef/Slot/HoleM62.csv
# WARNING! All changes made in this file will be lost!
"""Method code available at https://github.com/Eomys/pyleecan/tree/master/pyleecan/Methods/Slot/HoleM62
"""
...
try:
    from ..Methods.Slot.HoleM62._comp_point_coordinate import _comp_point_coordinate
except ImportError as error:
    _comp_point_coordinate = error

try:
    from ..Methods.Slot.HoleM62.build_geometry import build_geometry
except ImportError as error:
    build_geometry = error

try:
    from ..Methods.Slot.HoleM62.plot_schematics import plot_schematics
except ImportError as error:
...
try:
    from ..Methods.Slot.HoleM62.comp_radius import comp_radius
except ImportError as error:
    comp_radius = error

try:
    from ..Methods.Slot.HoleM62.has_magnet import has_magnet
except ImportError as error:
    has_magnet = error

try:
    from ..Methods.Slot.HoleM62.check import check
except ImportError as error:
    check = error

try:
    from ..Methods.Slot.HoleM62.comp_magnetization_dict import comp_magnetization_dict
except ImportError as error:
    comp_magnetization_dict = error

try:
    from ..Methods.Slot.HoleM62.comp_surface_magnet_id import comp_surface_magnet_id
except ImportError as error:
    comp_surface_magnet_id = error

try:
    from ..Methods.Slot.HoleM62.comp_surface import comp_surface
except ImportError as error:
    comp_surface = error
...
class HoleM62(HoleMag):
    ...
    # cf Methods.Slot.HoleM62._comp_point_coordinate
    if isinstance(_comp_point_coordinate, ImportError):
        _comp_point_coordinate = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM62 method _comp_point_coordinate: "
    ...
    # cf Methods.Slot.HoleM62.build_geometry
    if isinstance(build_geometry, ImportError):
        build_geometry = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM62 method build_geometry: " + str(build_geometry)
                )
            )
        )
    else:
        build_geometry = build_geometry
    # cf Methods.Slot.HoleM62.plot_schematics
    if isinstance(plot_schematics, ImportError):
        plot_schematics = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM62 method plot_schematics: " + str(plot_schematics)
                )
            )
        )
    else:
    ...
    if isinstance(remove_magnet, ImportError):
        remove_magnet = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM62 method remove_magnet: " + str(remove_magnet)
                )
            )
        )
    else:
        remove_magnet = remove_magnet
    # cf Methods.Slot.HoleM62.comp_radius
    if isinstance(comp_radius, ImportError):
        comp_radius = property(
            fget=lambda x: raise_(
                ImportError("Can't use HoleM62 method comp_radius: " + str(comp_radius))
            )
        )
    else:
        comp_radius = comp_radius
    # cf Methods.Slot.HoleM62.has_magnet
    if isinstance(has_magnet, ImportError):
        has_magnet = property(
            fget=lambda x: raise_(
                ImportError("Can't use HoleM62 method has_magnet: " + str(has_magnet))
            )
        )
    else:
        has_magnet = has_magnet
    # cf Methods.Slot.HoleM62.check
    if isinstance(check, ImportError):
        check = property(
            fget=lambda x: raise_(
                ImportError("Can't use HoleM62 method check: " + str(check))
            )
        )
    else:
        check = check
    # cf Methods.Slot.HoleM62.comp_magnetization_dict
    if isinstance(comp_magnetization_dict, ImportError):
        comp_magnetization_dict = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM62 method comp_magnetization_dict: "
    ...
    # cf Methods.Slot.HoleM62.comp_surface_magnet_id
    if isinstance(comp_surface_magnet_id, ImportError):
        comp_surface_magnet_id = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM62 method comp_surface_magnet_id: "
    ...
    # cf Methods.Slot.HoleM62.comp_surface
    if isinstance(comp_surface, ImportError):
        comp_surface = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM62 method comp_surface: " + str(comp_surface)
                )
            )
        )
    else:
    ...
    def __str__(self):
        ...
        HoleM62_str += super(HoleM62, self).__str__()
        HoleM62_str += "W0 = " + str(self.W0) + linesep
        HoleM62_str += "H0 = " + str(self.H0) + linesep
        HoleM62_str += "H1 = " + str(self.H1) + linesep
        HoleM62_str += "W0_is_rad = " + str(self.W0_is_rad) + linesep
        if self.magnet_0 is not None:
            tmp = self.magnet_0.__str__().replace(linesep, linesep + "\t").rstrip("\t")
            HoleM62_str += "magnet_0 = " + tmp
        else:
            HoleM62_str += "magnet_0 = None" + linesep + linesep
        return HoleM62_str
    ...
    def __eq__(self, other):
        ...
        if not super(HoleM62, self).__eq__(other):
            return False
        if other.W0 != self.W0:
            return False
        if other.H0 != self.H0:
    ...
    def compare(self, other, name="self", ignore_list=None, is_add_value=False):
        ...
        if other._W0_is_rad != self._W0_is_rad:
            if is_add_value:
                val_str = (
                    " (self="
                    + str(self._W0_is_rad)
                    + ", other="
                    + str(other._W0_is_rad)
                    + ")"
                )
                diff_list.append(name + ".W0_is_rad" + val_str)
            else:
                diff_list.append(name + ".W0_is_rad")
    ...
    def __sizeof__(self):
        ...
        S += super(HoleM62, self).__sizeof__()
        S += getsizeof(self.W0)
        S += getsizeof(self.H0)
        S += getsizeof(self.H1)
        S += getsizeof(self.W0_is_rad)
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        # Get the properties inherited from HoleMag
        HoleM62_dict = super(HoleM62, self).as_dict(
            type_handle_ndarray=type_handle_ndarray,
            keep_function=keep_function,
            **kwargs
        )
        HoleM62_dict["W0"] = self.W0
        HoleM62_dict["H0"] = self.H0
        HoleM62_dict["H1"] = self.H1
        HoleM62_dict["W0_is_rad"] = self.W0_is_rad
        if self.magnet_0 is None:
            HoleM62_dict["magnet_0"] = None
        else:
            HoleM62_dict["magnet_0"] = self.magnet_0.as_dict(
                type_handle_ndarray=type_handle_ndarray,
                keep_function=keep_function,
                **kwargs
            )
        ...
        HoleM62_dict["__class__"] = "HoleM62"
        return HoleM62_dict
