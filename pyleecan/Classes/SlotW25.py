...
try:
    from ..Methods.Slot.SlotW25.get_surface_wind import get_surface_wind
except ImportError as error:
...
class SlotW25(SlotWind):
    ...
    # cf Methods.Slot.SlotW25.get_surface_wind
    if isinstance(get_surface_wind, ImportError):
        get_surface_wind = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW25 method get_surface_wind: " + str(get_surface_wind)
                )
            )
        )
    else:
    ...
    def __init__(self, W3=0.003, H2=0.003, W4=0.003, H1=0.003, Zs=36, init_dict = None, init_str = None):
