...
try:
    from ..Methods.Slot.SlotW26.get_surface_wind import get_surface_wind
except ImportError as error:
...
class SlotW26(SlotWind):
    ...
    # cf Methods.Slot.SlotW26.get_surface_wind
    if isinstance(get_surface_wind, ImportError):
        get_surface_wind = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW26 method get_surface_wind: " + str(get_surface_wind)
                )
            )
        )
    else:
    ...
    if isinstance(comp_height_wind, ImportError):
        comp_height_wind = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW26 method comp_height_wind: " + str(comp_height_wind)
                )
            )
        )
    else:
    ...
    def __init__(self, W0=0.0122, H0=0.001, H1=0.003, R1=0.003, R2=0.003, Zs=36, init_dict = None, init_str = None):
