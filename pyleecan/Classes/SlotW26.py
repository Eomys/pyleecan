...
try:
    from ..Methods.Slot.SlotW26._comp_line_dict import _comp_line_dict
...
try:
    from ..Methods.Slot.SlotW26.comp_surface_opening import comp_surface_opening
except ImportError as error:
...
try:
    from ..Methods.Slot.SlotW26.get_surface_opening import get_surface_opening
except ImportError as error:
...
class SlotW26(Slot):
    ...
    # cf Methods.Slot.SlotW26._comp_line_dict
    if isinstance(_comp_line_dict, ImportError):
        _comp_line_dict = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW26 method _comp_line_dict: " + str(_comp_line_dict)
                )
            )
        )
    else:
    ...
    # cf Methods.Slot.SlotW26.comp_surface_opening
    if isinstance(comp_surface_opening, ImportError):
        comp_surface_opening = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW26 method comp_surface_opening: "
    ...
    # cf Methods.Slot.SlotW26.get_surface_opening
    if isinstance(get_surface_opening, ImportError):
        get_surface_opening = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW26 method get_surface_opening: "
    ...
    def __init__(self, W0=0.0122, H0=0.001, H1=0.003, R1=0.003, R2=0.003, Zs=36, wedge_mat=None, is_bore=True, init_dict = None, init_str = None):
        ...
        self.R2 = R2
        # Call Slot init
        super(SlotW26, self).__init__(Zs=Zs, wedge_mat=wedge_mat, is_bore=is_bore)
    ...
    def compare(self, other, name='self', ignore_list=None, is_add_value=False):
        ...
        # Check the properties inherited from Slot
        diff_list.extend(super(SlotW26, self).compare(other,name=name, ignore_list=ignore_list, is_add_value=is_add_value))
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        # Get the properties inherited from Slot
        SlotW26_dict = super(SlotW26, self).as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs)
    ...
    def copy(self):
        ...
        obj_copy = type(self)(W0=W0_val,H0=H0_val,H1=H1_val,R1=R1_val,R2=R2_val,Zs=Zs_val,wedge_mat=wedge_mat_val,is_bore=is_bore_val)
        return obj_copy
