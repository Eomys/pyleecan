...
class ImportMatrixXls(ImportMatrix):
    ...
    def copy(self):
        ...
        sheet_val = self.sheet
        skiprows_val = self.skiprows
        usecols_val = self.usecols
        if self.axes_colrows is None:
            axes_colrows_val = None
        else:
            axes_colrows_val = self.axes_colrows.copy()
