...
class SlotW60(SlotWind):
    ...
    if isinstance(get_surface_wind, ImportError):
        get_surface_wind = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW60 method get_surface_wind: " + str(get_surface_wind)
                )
            )
        )
    else:
    ...
    def __init__(self, W1=0.02, W2=0.03, H1=0.05, H2=0.15, R1=0.03, H3=0, H4=0, W3=0, Zs=36, init_dict = None, init_str = None):
