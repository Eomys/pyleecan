...
class WindingUD(Winding):
    ...
    if isinstance(import_from_csv, ImportError):
        import_from_csv = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use WindingUD method import_from_csv: " + str(import_from_csv)
                )
            )
        )
    else:
    ...
    def compare(self, other, name='self', ignore_list=None, is_add_value=False):
        ...
        # Check the properties inherited from Winding
        diff_list.extend(super(WindingUD, self).compare(other,name=name, ignore_list=ignore_list, is_add_value=is_add_value))
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        # Get the properties inherited from Winding
        WindingUD_dict = super(WindingUD, self).as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs)
