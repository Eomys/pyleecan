...
class Winding(FrozenClass):
    ...
    if isinstance(comp_periodicity, ImportError):
        comp_periodicity = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use Winding method comp_periodicity: " + str(comp_periodicity)
                )
            )
        )
    else:
    ...
    def __str__(self):
        ...
        Winding_str += "wind_mat = " + linesep + str(self.wind_mat).replace(linesep, linesep + "\t") + linesep + linesep
        ...
        if self.end_winding is not None:
            tmp = self.end_winding.__str__().replace(linesep, linesep + "\t").rstrip("\t")
        ...
        Winding_str += "dual_tri_phase_shift = " + str(self.dual_tri_phase_shift) + linesep
    ...
    def compare(self, other, name='self', ignore_list=None, is_add_value=False):
        ...
        if other._is_reverse_wind != self._is_reverse_wind:
            if is_add_value:
                val_str = ' (self='+str(self._is_reverse_wind)+', other='+str(other._is_reverse_wind)+')'
        ...
        if other._Nslot_shift_wind != self._Nslot_shift_wind:
            if is_add_value:
                val_str = ' (self='+str(self._Nslot_shift_wind)+', other='+str(other._Nslot_shift_wind)+')'
        ...
        if other._Lewout is not None and self._Lewout is not None and isnan(other._Lewout) and isnan(self._Lewout):
            pass
        elif other._Lewout != self._Lewout:
            if is_add_value:
                val_str = ' (self='+str(self._Lewout)+', other='+str(other._Lewout)+')'
                diff_list.append(name+'.Lewout'+val_str)
            else:
                diff_list.append(name+'.Lewout')
        if (other.conductor is None and self.conductor is not None) or (other.conductor is not None and self.conductor is None):
            diff_list.append(name+'.conductor None mismatch')
        elif self.conductor is not None:
            diff_list.extend(self.conductor.compare(other.conductor,name=name+'.conductor',ignore_list=ignore_list,is_add_value=is_add_value))
        ...
        if other._Nlayer != self._Nlayer:
            if is_add_value:
                val_str = ' (self='+str(self._Nlayer)+', other='+str(other._Nlayer)+')'
        ...
        if other._is_aper_a != self._is_aper_a:
            if is_add_value:
                val_str = ' (self='+str(self._is_aper_a)+', other='+str(other._is_aper_a)+')'
                diff_list.append(name+'.is_aper_a'+val_str)
            else:
                diff_list.append(name+'.is_aper_a')
        if (other.end_winding is None and self.end_winding is not None) or (other.end_winding is not None and self.end_winding is None):
            diff_list.append(name+'.end_winding None mismatch')
        elif self.end_winding is not None:
            diff_list.extend(self.end_winding.compare(other.end_winding,name=name+'.end_winding',ignore_list=ignore_list,is_add_value=is_add_value))
        if other._is_reverse_layer != self._is_reverse_layer:
            if is_add_value:
                val_str = ' (self='+str(self._is_reverse_layer)+', other='+str(other._is_reverse_layer)+')'
        ...
        if other._is_change_layer != self._is_change_layer:
            if is_add_value:
                val_str = ' (self='+str(self._is_change_layer)+', other='+str(other._is_change_layer)+')'
        ...
        if other._is_permute_B_C != self._is_permute_B_C:
            if is_add_value:
                val_str = ' (self='+str(self._is_permute_B_C)+', other='+str(other._is_permute_B_C)+')'
                diff_list.append(name+'.is_permute_B_C'+val_str)
            else:
                diff_list.append(name+'.is_permute_B_C')
        if other._dual_tri_phase_shift is not None and self._dual_tri_phase_shift is not None and isnan(other._dual_tri_phase_shift) and isnan(self._dual_tri_phase_shift):
            pass
        elif other._dual_tri_phase_shift != self._dual_tri_phase_shift:
            if is_add_value:
                val_str = ' (self='+str(self._dual_tri_phase_shift)+', other='+str(other._dual_tri_phase_shift)+')'
        ...
        if other._is_wye != self._is_wye:
            if is_add_value:
                val_str = ' (self='+str(self._is_wye)+', other='+str(other._is_wye)+')'
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        if self.conductor is None:
            ...
            Winding_dict["conductor"] = self.conductor.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs)
        ...
        if self.end_winding is None:
            ...
            Winding_dict["end_winding"] = self.end_winding.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs)
    ...
    def _set_end_winding(self, value):
        ...
        if isinstance(value, str):  # Load from file
            try:
                ...
                value = None
        if isinstance(value, dict) and '__class__' in value:
            class_obj = import_class('pyleecan.Classes', value.get('__class__'), 'end_winding')
            value = class_obj(init_dict=value)
        elif type(value) is int and value == -1:  # Default constructor
