...
try:
    from ..Methods.Slot.SlotW21._comp_line_list import _comp_line_list
...
class SlotW21(Slot):
    ...
    # cf Methods.Slot.SlotW21._comp_line_list
    if isinstance(_comp_line_list, ImportError):
        _comp_line_list = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW21 method _comp_line_list: " + str(_comp_line_list)
                )
            )
        )
    else:
