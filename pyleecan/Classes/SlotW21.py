...
try:
    from ..Methods.Slot.SlotW21._comp_line_dict import _comp_line_dict
...
try:
    from ..Methods.Slot.SlotW21.comp_surface_opening import comp_surface_opening
except ImportError as error:
...
try:
    from ..Methods.Slot.SlotW21.get_surface_opening import get_surface_opening
except ImportError as error:
...
class SlotW21(Slot):
    ...
    # cf Methods.Slot.SlotW21._comp_line_dict
    if isinstance(_comp_line_dict, ImportError):
        _comp_line_dict = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW21 method _comp_line_dict: " + str(_comp_line_dict)
                )
            )
        )
    else:
    ...
    # cf Methods.Slot.SlotW21.comp_surface_opening
    if isinstance(comp_surface_opening, ImportError):
        comp_surface_opening = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW21 method comp_surface_opening: "
    ...
    # cf Methods.Slot.SlotW21.get_surface_opening
    if isinstance(get_surface_opening, ImportError):
        get_surface_opening = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW21 method get_surface_opening: "
    ...
    def __init__(self, W0=0.003, H0=0.003, H1=0, H1_is_rad=False, W1=0.013, H2=0.02, W2=0.01, Zs=36, wedge_mat=None, is_bore=True, init_dict = None, init_str = None):
        ...
        self.W2 = W2
        # Call Slot init
        super(SlotW21, self).__init__(Zs=Zs, wedge_mat=wedge_mat, is_bore=is_bore)
    ...
    def compare(self, other, name='self', ignore_list=None, is_add_value=False):
        ...
        # Check the properties inherited from Slot
        diff_list.extend(super(SlotW21, self).compare(other,name=name, ignore_list=ignore_list, is_add_value=is_add_value))
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        # Get the properties inherited from Slot
        SlotW21_dict = super(SlotW21, self).as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs)
    ...
    def copy(self):
        ...
        obj_copy = type(self)(W0=W0_val,H0=H0_val,H1=H1_val,H1_is_rad=H1_is_rad_val,W1=W1_val,H2=H2_val,W2=W2_val,Zs=Zs_val,wedge_mat=wedge_mat_val,is_bore=is_bore_val)
        return obj_copy
