...
try:
    from ..Methods.Mesh.RefSegmentP1.interpolation import interpolation
except ImportError as error:
    interpolation = error
...
class RefSegmentP1(RefCell):
    ...
    # cf Methods.Mesh.RefSegmentP1.interpolation
    if isinstance(interpolation, ImportError):
        interpolation = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use RefSegmentP1 method interpolation: " + str(interpolation)
                )
            )
        )
    else:
