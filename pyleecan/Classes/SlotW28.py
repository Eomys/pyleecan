...
class SlotW28(SlotWind):
    ...
    if isinstance(get_surface_wind, ImportError):
        get_surface_wind = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW28 method get_surface_wind: " + str(get_surface_wind)
                )
            )
        )
    else:
    ...
    if isinstance(comp_height_wind, ImportError):
        comp_height_wind = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW28 method comp_height_wind: " + str(comp_height_wind)
                )
            )
        )
    else:
    ...
    def __init__(self, W0=0.0122, H0=0.001, R1=0.003, W3=0.005, H3=0.003, Zs=36, init_dict = None, init_str = None):
