...
class OptiProblem(FrozenClass):
    ...
    def compare(self, other, name='self', ignore_list=None, is_add_value=False):
        ...
        if (other.design_var is None and self.design_var is not None) or (other.design_var is not None and self.design_var is None):
            ...
            for ii in range(len(other.design_var)):
                diff_list.extend(self.design_var[ii].compare(other.design_var[ii],name=name+'.design_var['+str(ii)+']',ignore_list=ignore_list,is_add_value=is_add_value))
        if (other.obj_func is None and self.obj_func is not None) or (other.obj_func is not None and self.obj_func is None):
            ...
            for ii in range(len(other.obj_func)):
                diff_list.extend(self.obj_func[ii].compare(other.obj_func[ii],name=name+'.obj_func['+str(ii)+']',ignore_list=ignore_list,is_add_value=is_add_value))
        if other._eval_func_str != self._eval_func_str:
            diff_list.append(name+'.eval_func')
        if (other.constraint is None and self.constraint is not None) or (other.constraint is not None and self.constraint is None):
            ...
            for ii in range(len(other.constraint)):
                diff_list.extend(self.constraint[ii].compare(other.constraint[ii],name=name+'.constraint['+str(ii)+']',ignore_list=ignore_list,is_add_value=is_add_value))
        if other._preprocessing_str != self._preprocessing_str:
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        if self.simu is None:
            ...
            OptiProblem_dict["simu"] = self.simu.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs)
        if self.design_var is None:
            ...
            for obj in self.design_var:
                if obj is not None:
                    OptiProblem_dict['design_var'].append(obj.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs))
                else:
        ...
        if self.obj_func is None:
            ...
            for obj in self.obj_func:
                if obj is not None:
                    OptiProblem_dict['obj_func'].append(obj.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs))
                else:
        ...
        if self._eval_func_str is not None:
            ...
            if self.eval_func is not None:
                self.get_logger().warning(
                    "OptiProblem.as_dict(): " +f"Function {self.eval_func.__name__} is not serializable " + "and will be converted to None."
                )
        if self.constraint is None:
            ...
            for obj in self.constraint:
                if obj is not None:
                    OptiProblem_dict['constraint'].append(obj.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs))
                else:
        ...
        if self._preprocessing_str is not None:
            ...
            if self.preprocessing is not None:
                self.get_logger().warning(
                    "OptiProblem.as_dict(): " +f"Function {self.preprocessing.__name__} is not serializable " + "and will be converted to None."
                )
        if self.datakeeper_list is None:
            ...
            for obj in self.datakeeper_list:
                if obj is not None:
                    OptiProblem_dict['datakeeper_list'].append(obj.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs))
                else:
    ...
    def copy(self):
        ...
        obj_copy = type(self)(simu=simu_val,design_var=design_var_val,obj_func=obj_func_val,eval_func=eval_func_val,constraint=constraint_val,preprocessing=preprocessing_val,datakeeper_list=datakeeper_list_val)
        return obj_copy
