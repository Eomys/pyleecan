...
try:
    from ..Methods.Machine.LamHoleNS.comp_periodicity_spatial import comp_periodicity_spatial
except ImportError as error:
...
class LamHoleNS(LamH):
    ...
    if isinstance(_plot_arrow_mag, ImportError):
        _plot_arrow_mag = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use LamHoleNS method _plot_arrow_mag: " + str(_plot_arrow_mag)
                )
            )
        )
    else:
    ...
    def __init__(self, hole_north=-1, hole_south=-1, L1=0.35, mat_type=-1, Nrvd=0, Wrvd=0, Kf1=0.95, is_internal=True, Rint=0, Rext=1, is_stator=True, axial_vent=-1, notch=-1, skew=None, bore=None, yoke=None, init_dict = None, init_str = None):
        ...
        # Call LamH init
        super(LamHoleNS, self).__init__(L1=L1, mat_type=mat_type, Nrvd=Nrvd, Wrvd=Wrvd, Kf1=Kf1, is_internal=is_internal, Rint=Rint, Rext=Rext, is_stator=is_stator, axial_vent=axial_vent, notch=notch, skew=skew, bore=bore, yoke=yoke)
    ...
    def __str__(self):
        ...
        for ii in range(len(self.hole_north)):
            tmp = self.hole_north[ii].__str__().replace(linesep, linesep + "\t") + linesep
        ...
        for ii in range(len(self.hole_south)):
            tmp = self.hole_south[ii].__str__().replace(linesep, linesep + "\t") + linesep
    ...
    def compare(self, other, name='self', ignore_list=None, is_add_value=False):
        ...
        # Check the properties inherited from LamH
        diff_list.extend(super(LamHoleNS, self).compare(other,name=name, ignore_list=ignore_list, is_add_value=is_add_value))
        if (other.hole_north is None and self.hole_north is not None) or (other.hole_north is not None and self.hole_north is None):
            ...
            for ii in range(len(other.hole_north)):
                diff_list.extend(self.hole_north[ii].compare(other.hole_north[ii],name=name+'.hole_north['+str(ii)+']',ignore_list=ignore_list,is_add_value=is_add_value))
        if (other.hole_south is None and self.hole_south is not None) or (other.hole_south is not None and self.hole_south is None):
            ...
            for ii in range(len(other.hole_south)):
                diff_list.extend(self.hole_south[ii].compare(other.hole_south[ii],name=name+'.hole_south['+str(ii)+']',ignore_list=ignore_list,is_add_value=is_add_value))
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        # Get the properties inherited from LamH
        LamHoleNS_dict = super(LamHoleNS, self).as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs)
        if self.hole_north is None:
            ...
            for obj in self.hole_north:
                if obj is not None:
                    LamHoleNS_dict['hole_north'].append(obj.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs))
                else:
        ...
        if self.hole_south is None:
            ...
            for obj in self.hole_south:
                if obj is not None:
                    LamHoleNS_dict['hole_south'].append(obj.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs))
                else:
            ...
            for ii, obj in enumerate(value):
                if isinstance(obj, str):  # Load from file
                    try:
                        ...
                        value[ii] = None
                if type(obj) is dict:
                    class_obj = import_class('pyleecan.Classes', obj.get('__class__'), 'hole_north')
            ...
            for ii, obj in enumerate(value):
                if isinstance(obj, str):  # Load from file
                    try:
                        ...
                        value[ii] = None
                if type(obj) is dict:
                    class_obj = import_class('pyleecan.Classes', obj.get('__class__'), 'hole_south')
                    value[ii] = class_obj(init_dict=obj)
                if value[ii] is not None:
                    value[ii].parent = self
        if value == -1:
