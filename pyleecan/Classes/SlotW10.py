...
class SlotW10(SlotWind):
    ...
    if isinstance(get_surface_wind, ImportError):
        get_surface_wind = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW10 method get_surface_wind: " + str(get_surface_wind)
                )
            )
        )
    else:
    ...
    def __init__(self, W0=0.0122, H0=0.001, H1=0.0015, W1=0.014, H2=0.0325, W2=0.0122, H1_is_rad=False, Zs=36, init_dict = None, init_str = None):
