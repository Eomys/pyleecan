...
try:
    from ..Methods.Slot.SlotW12.get_surface_wind import get_surface_wind
except ImportError as error:
...
class SlotW12(SlotWind):
    ...
    # cf Methods.Slot.SlotW12.get_surface_wind
    if isinstance(get_surface_wind, ImportError):
        get_surface_wind = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW12 method get_surface_wind: " + str(get_surface_wind)
                )
            )
        )
    else:
    ...
    def __init__(self, H0=0.003, H1=0, R1=0.001, R2=0.001, Zs=36, init_dict = None, init_str = None):
