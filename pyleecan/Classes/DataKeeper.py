...
class DataKeeper(FrozenClass):
    ...
    def __eq__(self, other):
        ...
        if other.result != self.result:
            return False
        if isinstance(self.result_ref, np.ndarray) and not np.array_equal(other.result_ref, self.result_ref):
            return False
    ...
    def compare(self, other, name='self', ignore_list=None, is_add_value=False):
        ...
        if (other.result is None and self.result is not None) or (other.result is not None and self.result is None):
            ...
            for ii in range(len(other.result)):
                if hasattr(self.result[ii], 'compare'):
                    diff_list.extend(self.result[ii].compare(other.result[ii],name=name+'.result['+str(ii)+']',ignore_list=ignore_list,is_add_value=is_add_value))
                elif other._result[ii] != self._result[ii]:
                    diff_list.append(name+'.result['+str(ii)+'])')
        if (other.result_ref is None and self.result_ref is not None) or (other.result_ref is not None and self.result_ref is None):
            diff_list.append(name+'.result_ref')
        elif self.result_ref is None:
            pass
        elif isinstance(self.result_ref, np.ndarray) and not np.array_equal(other.result_ref, self.result_ref):
            diff_list.append(name+'.result_ref')
        elif hasattr(self.result_ref, 'compare'):
            diff_list.extend(self.result_ref.compare(other.result_ref,name=name+'.result_ref',ignore_list=ignore_list,is_add_value=is_add_value))
        elif other._result_ref != self._result_ref:
            diff_list.append(name+'.result_ref')
        if other._physic != self._physic:
            if is_add_value:
                val_str = ' (self='+str(self._physic)+', other='+str(other._physic)+')'
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        if self._keeper_str is not None:
            ...
            if self.keeper is not None:
                self.get_logger().warning(
                    "DataKeeper.as_dict(): " +f"Function {self.keeper.__name__} is not serializable " + "and will be converted to None."
                )
        if self._error_keeper_str is not None:
            ...
            if self.error_keeper is not None:
                self.get_logger().warning(
                    "DataKeeper.as_dict(): " +f"Function {self.error_keeper.__name__} is not serializable " + "and will be converted to None."
                )
        if self.result is None:
            ...
            for obj in self.result:
                if obj is None:
                    ...
                    DataKeeper_dict['result'].append(obj.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs))
        ...
        if self.result_ref is None:
            ...
            DataKeeper_dict["result_ref"] = self.result_ref.as_dict(type_handle_ndarray=type_handle_ndarray, keep_function=keep_function, **kwargs)
        else:
            DataKeeper_dict["result_ref"] = self.result_ref
        ...
        if type(value) is list:
            for ii, obj in enumerate(value):
                ...
                if type(obj) is dict and '__class__' in obj:  # pyleecan object
                    try:
                        class_obj = import_class('SciDataTool.Classes', obj.get('__class__'), 'result')
                    except Exception:
                        class_obj = import_class('pyleecan.Classes', obj.get('__class__'), 'result')
    ...
    def _set_result_ref(self, value):
        ...
        if isinstance(value, dict) and '__class__' in value:
            try:
                class_obj = import_class('pyleecan.Classes', value.get('__class__'), 'result_ref')
            except:
                class_obj = import_class('SciDataTool.Classes', value.get('__class__'), 'result_ref')
