...
class EndWinding(FrozenClass):
    ...
    def compare(self, other, name='self', ignore_list=None, is_add_value=False):
        ...
        if type(other) != type(self):
            return ['type('+name+')']
        diff_list = list()
        if other._Lew_enforced is not None and self._Lew_enforced is not None and isnan(other._Lew_enforced) and isnan(self._Lew_enforced):
            pass
        elif other._Lew_enforced != self._Lew_enforced:
            if is_add_value:
                val_str = ' (self='+str(self._Lew_enforced)+', other='+str(other._Lew_enforced)+')'
