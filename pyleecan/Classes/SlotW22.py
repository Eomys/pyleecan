...
class SlotW22(SlotWind):
    ...
    if isinstance(get_surface_wind, ImportError):
        get_surface_wind = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotW22 method get_surface_wind: " + str(get_surface_wind)
                )
            )
        )
    else:
    ...
    def __init__(self, W0=0.043633, H0=0.006, H2=0.04, W2=0.08725, Zs=36, init_dict = None, init_str = None):
