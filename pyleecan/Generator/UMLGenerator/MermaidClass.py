...
class MermaidClass():
    ...
    def __init__(self,file_path):
        ...
        self.df=pd.read_csv(file_path,usecols=[0,4,10,11,14],
                            names=["Variable name", "Type", "Inherit", "Methods", "Class description"],
                            header=0)
        self.variables=self.df["Variable name"].dropna().to_list() if not pd.isnull(self.df["Variable name"][0]) else []
        self.variable_types=self.df["Type"].dropna().to_list() if not pd.isnull(self.df["Type"][0]) else []
        self.methods=self.df["Methods"].dropna().to_list() #if not pd.isnull(self.df["Methods"][0]) else []
        self.parent=self.df["Inherit"][0] if not pd.isnull(self.df["Inherit"][0]) else None
        try:
            self.is_abstract=True if "Abstract" in self.df["Class description"][0] else False
